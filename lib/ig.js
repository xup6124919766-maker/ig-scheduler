...
// 對暫時性錯誤（5xx / 429 / 網路）做指數退避重試 — 4xx 例外不重試（如 token 失效）
const RETRYABLE_STATUS = new Set([429, 500, 502, 503, 504]);
const isRetryable = (err) => {
  if (!err.response) return true; // 網路錯誤、timeout、DNS 失敗
  return RETRYABLE_STATUS.has(err.response.status);
};

const requestWithRetry = async (method, url, opts = {}, { maxRetries = 3, baseDelayMs = 800 } = {}) => {
  let lastErr;
...
export class IGClient {
  ...
  async _get(path, params = {}) {
    const { data } = await requestWithRetry('get', `${this.graph}${path}`, {
  ...
  async _post(path, params = {}) {
    const { data } = await requestWithRetry('post', `${this.graph}${path}`, {
      data: null,
      params: { access_token: this.token, ...params },
    });
    return data;
  ...
  async resolveAccount() {
    ...
    const { data: me } = await axios.get(`${GRAPH_FB(this.version)}/me/accounts`, {
      params: { access_token: this.token, fields: 'id,name,access_token,instagram_business_account' },
    });
    ...
    // 🎯 page.access_token 是「永不過期 Page Token」（前提：原始 user token 是長效）
    this.tokenType = page.access_token ? 'fb_page' : 'fb';
    return {
      pageId: page.id,
      pageName: page.name,
