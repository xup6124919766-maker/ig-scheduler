...
// 累加重試次數並回傳新值（給排程層判斷是否還要自動重試）
export const incrementRetry = (id) => {
  db.prepare('UPDATE posts SET retry_count = retry_count + 1 WHERE id = ?').run(id);
  return db.prepare('SELECT retry_count FROM posts WHERE id = ?').get(id)?.retry_count ?? 0;
};

// 暫時性錯誤 → 退回 pending 並延後 scheduled_at，讓下一輪 tick 自動重發
export const requeuePost = (id, nextAt, lastError = null) =>
  db.prepare("UPDATE posts SET status='pending', scheduled_at=?, error=? WHERE id=?")
    .run(nextAt, lastError, id);
